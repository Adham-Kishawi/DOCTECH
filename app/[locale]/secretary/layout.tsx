...
import { LayoutDashboard, Calendar, CalendarCheck, BarChart2, MessageSquare, Bell, Users, User, MessageCircle } from "lucide-react";
    ...
    <div className="flex min-h-screen" style={{ background: "var(--color-background)" }}>
      <aside className="fixed inset-y-0 left-0 flex flex-col z-30"
        ...
        <nav className="flex-1 py-4 px-2 space-y-1 overflow-y-auto">
          {navItems.map(({ href, label, icon: Icon }) => {
            ...
            return (
              <Link key={href} href={`/${locale}/secretary/${href}`}
                className={cn("flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all")}
                style={isActive ? { background: ACCENT, color: "white" } : { color: "var(--color-text-secondary)" }}>
      ...
      <div className="flex-1 flex flex-col" style={{ marginLeft: "var(--sidebar-width)" }}>
        <header className="sticky top-0 z-20 flex items-center justify-between px-6 h-14"
          ...
          <span className="text-sm font-medium" style={{ color: "var(--color-text-secondary)" }}>Secretary View</span>
        </header>
